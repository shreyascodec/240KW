/**
 * Design Flow API Service
 * Handles GET and POST requests for each step of the design flow
 */

const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000/api'
const STORAGE_KEY = 'design_flow_data'

/**
 * Generate a unique session ID for this design flow
 */
const getSessionId = () => {
...
export const getStepData = async (stepId) => {
  try {
    ...
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/design/step/${stepId}?sessionId=${sessionId}`, {
        method: 'GET',
        headers: {
...
export const postStepData = async (stepId, stepData) => {
  try {
    ...
    // Try API first
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/design/step/${stepId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
export const getAllFormData = async () => {
  try {
    ...
    // Try API first
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/design/data?sessionId=${sessionId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
...
export const submitFormData = async (formData) => {
  try {
    ...
    // Try API first
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/design/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    const sessionId = getSessionId()
    localStorage.removeItem(`${STORAGE_KEY}_${sessionId}`)
    return { success: true, data: { message: 'Design form submitted successfully' } }
  } catch (error) {
