/**
 * Debugging Flow API Service
 * Handles GET and POST requests for each step of the debugging flow
 */

const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000/api'
const STORAGE_KEY = 'debugging_flow_data'

const getSessionId = () => {
  let sessionId = sessionStorage.getItem('debugging_session_id')
  if (!sessionId) {
    sessionId = `debugging_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    sessionStorage.setItem('debugging_session_id', sessionId)
  }
  return sessionId
}
...
export const getStepData = async (stepId) => {
  try {
    ...
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/debugging/step/${stepId}?sessionId=${sessionId}`, {
        method: 'GET',
...
export const postStepData = async (stepId, stepData) => {
  try {
    ...
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/debugging/step/${stepId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const submitFormData = async (formData) => {
  try {
    ...
    if (API_BASE_URL && API_BASE_URL !== 'http://localhost:3000/api') {
      const response = await fetch(`${API_BASE_URL}/debugging/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    localStorage.removeItem(`${STORAGE_KEY}_${sessionId}`)
    return { success: true, data: { message: 'Debugging form submitted successfully' } }
  } catch (error) {
...
export const clearSessionData = () => {
  const sessionId = getSessionId()
  localStorage.removeItem(`${STORAGE_KEY}_${sessionId}`)
  sessionStorage.removeItem('debugging_session_id')
}
